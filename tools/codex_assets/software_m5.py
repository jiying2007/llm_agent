#!/usr/bin/env python3
"""Canonical production-qualified Software M5 certification.

This is the sole Software M5 implementation. It validates current signed
supply-chain evidence, measured runtime evidence, and a real independently
governed pilot repository using either a managed gitlink or an immutable
reference pin. Long-duration qualification remains a separate asset contract.
"""
...
def _reference_repo_pins(root: Path) -> dict[str, dict[str, Any]]:
    manifest = _load_object(
        _repo_path(root, "manifests/reference_pins.json", "reference pin registry"),
...
def _pilot_start_matches_pin(
    ...
    for relative in event.get("evidence", []):
        if not isinstance(relative, str) or not relative.endswith(".json"):
            continue
        evidence = _load_object(_repo_path(root, relative, "field evidence"), "field evidence")
...
def _validate_field(root: Path, policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ledger = _load_object(
        _repo_path(root, field["ledger"], "M5 pilot ledger"), "M5 pilot ledger"
    )
    ...
    gitlinks = _load_object(
        _repo_path(root, "manifests/gitlinks.json", "gitlink registry"), "gitlink registry"
    )
    managed_paths = {
        item.get("path")
    ...
    events = _read_events(
        root, _repo_path(root, field["event_log"], "M5 event log")
    )
