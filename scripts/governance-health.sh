...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<USAGE
usage: scripts/governance-health.sh [root] [--format markdown|json]
...
overall="pass"
for name in stale_refs subrepo_state evidence_bundle pilot_readiness fallback_sunset runtime_boundary; do
...
if rg -q '"replacement_score":6[0-9]' "${TMP_DIR}/fallback_sunset.out"; then
  top_actions+=("补 live health gap 与第二真实 pilot 后再推进 fallback candidate-sunset")
fi
