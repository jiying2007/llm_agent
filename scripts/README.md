...
WeChat 文章吸收账本脚本：`scripts/generate-wechat-intake-ledger.sh` / `scripts/check-wechat-intake-ledger.sh .`。generator 只读扫描 `wechat-articles/`，生成 `reports/wechat-article-intake.jsonl`、`reports/wechat-article-intake.manifest.json` 与 `reports/wechat-absorb-next-batch.md`；checker 在纯 checkout 中校验提交内 snapshot，在真实 corpus 存在时追加严格再生成对比，`--articles-dir <path> --require-corpus` 可显式要求 live-corpus。负向契约由 `tests/test_wechat_intake_ledger.sh` 覆盖；细则见 `docs/runbooks/wechat-article-absorption.md`。
