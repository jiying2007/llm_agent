...
# 父仓尚未提交 gitlink 时：lock 必须匹配 ADK worktree，且 worktree 必须继承已记录 gitlink
scripts/check-adk-lock.sh . --worktree-integration
...
当前状态一致性门禁：`scripts/check-current-status-consistency.sh . --summary-json` 将 `reports/current-status.md` 视为最近一次已验证提交基线，并校验 7 天 freshness、实时 subrepo state、`adk.lock`、`agent-dev-kit` gitlink、ADK worktree HEAD、runtime live evidence、Knowledge Hub promotion boundary 和架构报告状态；`agent_dev_kit_commit` 绑定当前 gitlink/lock，`agent_dev_kit_release_commit` 可单独锚定最近一次制品证据，且门禁要求发布提交是当前提交祖先、两者之间无 runtime 映射资产变化；阻断陈旧 `IN PROGRESS`、unexpected dirty、过期 baseline、ADK commit mismatch、无证据 live-applied 声明和 active promotion claims；回归入口为 `tests/test_current_status_consistency.sh`。
...
维护性增长预算：`scripts/check-maintainability-budgets.sh --summary-json` 从 `manifests/comprehensive_optimization_backlog.json` 读取 root/ADK 的静态预算与 churn、owner concentration、inactive asset evidence metrics，输出 baseline drift、结构化 availability、source/window/population 和 hotspot；无受审 source 时三轴必须为 `not-available/null`，不能无样本 pass。`scripts/generate-maintainability-evidence.sh` 可从真实 Git revision window 生成完整 population 的 churn `review-required` candidate，默认 dry-run，绝不自动更新 backlog 或自批为 reviewed。详见 `docs/runbooks/maintainability-evidence-candidate.md`。新增 root Python 治理逻辑进入 `tools.codex_assets`，shell 入口保持只定位 root、注入 `PYTHONPATH` 和转发参数。
...
外部实践统一吸收入口：`scripts/practice-intake.sh collect|check|queue|cycle|recommend`。provider 固定为 `github`、`gitlab`、`gitee`、`openai-official`、`anthropic-official`、`wechat` 和 `manual`；默认只读本地 fixture 或用户提供的 metadata，只有显式 `--allow-network` 才允许访问 manifest 中的 HTTPS allowlist。所有输出均为 metadata-only、`review-required`，不会 clone、注册、自动采纳、修改 ADK、写 live runtime 或发送外部消息。GitHub、GitLab、Gitee 以固定 `as_of` 生成确定性候选；Gitee 空结果记为 `degraded-empty`，不能解释成“没有候选”。契约和运行方式见 `docs/runbooks/external-practice-intake.md`，微信公众号边界见 `docs/runbooks/wechat-metadata-intake.md`。

经独立审查后的参考仓登记使用 `scripts/onboard-reference-repository.sh`；输入必须同时满足 `external-practice-candidate/v1` 与独立 `external-practice-decision/v1`，且决策为 `ADOPT`、目标为 `reference-repository`。命令默认只生成 metadata-only dry-run；显式 `--apply` 硬性要求 clean workspace、clean local Git source、HTTPS origin 与候选 URL 一致、HEAD 快照固定和 `local-submodule` materialization，并以事务方式同步 registry、adoption matrix JSON/JSONL、lifecycle 和 plan 证据，失败时回滚新 submodule 与 metadata。它不承担发现、评分、吸收、ADK 修改或 live 发布。登记门禁为 `scripts/check-reference-repository-registration.sh`；移除只允许 `scripts/plan-reference-repository-removal.sh` 生成带 artifact digest 的 dry-run，再由 `scripts/check-reference-repository-removal.sh` 校验，不实现自动删除。细则见 `docs/runbooks/reference-repository-lifecycle.md`。

常用验证：`rtk scripts/check-practice-intake.sh .`、`rtk scripts/check-reference-repository-registration.sh .`、`rtk scripts/check-reference-repository-removal.sh .`、`rtk scripts/check-runtime-health-adapters-fixtures.sh .`、`rtk scripts/check-runtime-target-evidence-index.sh .`、`rtk scripts/check-loop-readiness.sh .`、`rtk scripts/check-scale-engine-governance.sh .`、`rtk scripts/check-harness-loop-engineering.sh .`、`rtk tests/test_external_practice_intake.sh`、`rtk tests/test_reference_repository_registration.sh`、`rtk tests/test_reference_repository_removal.sh`、`rtk tests/test_runtime_health_adapters.sh`、`rtk tests/test_runtime_target_evidence_index.sh`、`rtk tests/test_runtime_target_evidence_package.sh`、`rtk tests/test_runtime_target_evidence_promotion.sh`。统一入口读取 `manifests/external_practice_sources.json`、`manifests/external_practice_cycle.json` 和 `schemas/external-practice-*.schema.json`；参考仓登记读取 `manifests/reference_repository_registration_policy.json`，移除读取 `manifests/reference_repository_removal_policy.json`。`fixtures/external-practice/` 覆盖 provider、退化、安全、幂等和登记负例；`fixtures/reference-repository/removal/` 只服务已登记参考仓的移除计划。历史 `reports/` 仅作 provenance，不是 active contract。active-reference/local-submodule lifecycle 条目仍校验 source URL/provider/branch/commit/retrieved_at、runtime boundaries、analysis/absorption/security/deep-assessment evidence 和本地 HEAD commit；其他 runtime、loop、scale 与 harness 合同保持各自门禁不变。
