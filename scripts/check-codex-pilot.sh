...
echo "[INFO] report=${REPORT} mode=${MODE}"
...
# ── Mode: full (evidence + coverage + scenario verification) ────────
...
case "${MODE}" in
  ...
  full)     check_full ;;
  *)
    echo "[FAIL] unknown mode: ${MODE} (expected: evidence|coverage|full)" >&2
    exit 1
    ;;
esac
