...
- P0 来源仓共 6 个：`OpenSpec`、`artifact-gated-agents`、`codex`、`agent-dev-kit`、`superpowers`、`superpowers-zh`。
...
- 证据：
  - `agent-dev-kit/docs/workflows.md`
  - `agent-dev-kit/scripts/workflow.sh`
  - `agent-dev-kit/tests/test_workflow.sh`
- 验证命令：
  - `rtk agent-dev-kit/tests/test_workflow.sh`

2. `superpowers-zh`（workflow-core）
- 压实能力：中文流程表达与触发语义规范化
- 证据：
  - `agent-dev-kit/AGENTS.md`
  - `agent-dev-kit/docs/workflows.md`
  - `agent-dev-kit/docs/agent-skill-catalog.md`
- 验证命令：
  - `rtk bash agent-dev-kit/scripts/validate_assets.sh --strict`
...
- 证据：
  - `agent-dev-kit/optional-skills/artifact-gated-lite/SKILL.md`
  - `agent-dev-kit/docs/runbooks/artifact-gated-delivery.md`
- 验证命令：
  - `rtk agent-dev-kit/tests/test_workflow.sh`
...
- `rtk bash agent-dev-kit/tests/run_all.sh` -> All tests passed
