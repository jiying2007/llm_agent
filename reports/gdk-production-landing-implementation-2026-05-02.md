...
- 日期：2026-05-02
- 范围：`global-dev-kit` 生产级运行入口、profile 分层、runtime routing、Evidence Index、pilot coverage、上游吸收门禁、生产安装治理
- 目标：把 gdk 从“压实版资产包”推进为 `~/.codex` 的生产级统一分发层。
...
- 新增 `global-dev-kit/scripts/evidence_index.sh` 与 `devkit.sh evidence` 入口。
- 新增 `global-dev-kit/tests/test_evidence_index.sh`。
...
- `check-gdk-harden-readiness.sh` 默认接入 runtime routing 与 upstream intake；`--require-pilot` 时接入 pilot coverage 字段检查。
...
- 新增 `global-dev-kit/scripts/check_profile_coherence.sh` 与 `test_profile_coherence.sh`，校验 profile 继承重复、未知引用、默认 profile 漂移。
...
- `pilot_full_coverage_ready=yes`：六类 pilot 已完成 gdk 自举试跑，并由脚本校验 artifact 与命令级证据。
- 本轮完成的是生产级落地基础设施与 gdk 自举试跑；真实业务任务样例仍需继续积累，不声明已完成所有团队场景的长期替代结论。
- `core` 继续保持精简，复杂能力通过 optional skill/profile 启用。

## 验证证据

- `rtk global-dev-kit/tests/test_install.sh` -> PASS
- `rtk global-dev-kit/tests/test_profile_coherence.sh` -> PASS
- `rtk global-dev-kit/tests/test_evidence_index.sh` -> PASS
- `rtk global-dev-kit/tests/test_change_governance.sh` -> PASS
- `rtk global-dev-kit/tests/test_workflow.sh` -> PASS
- `rtk scripts/check-runtime-routing.sh .` -> PASS
- `rtk scripts/check-codex-pilot-coverage.sh .` -> PASS（full coverage ready）
- `rtk bash -lc 'cd global-dev-kit && bash scripts/devkit.sh match --skill planning-execution-loop --scope optional-skill --text "复杂任务需要计划审查和执行检查点时"'` -> PASS
- `rtk bash -lc 'cd global-dev-kit && bash tests/test_evidence_index.sh'` -> PASS
- `rtk bash -lc 'cd global-dev-kit && bash scripts/devkit.sh validate --strict'` -> PASS
- `rtk bash -lc 'cd global-dev-kit && bash scripts/devkit.sh install --tool codex --target /tmp/gdk-release-pilot-codex --mode copy --profile release-hardening --with-optional-skill security-supply-chain --install-report /tmp/gdk-release-pilot-install.md --lock-version 0.3.0 --dry-run'` -> PASS
- `rtk bash -lc 'cd global-dev-kit && bash scripts/devkit.sh match --skill cross-team-handoff --scope optional-skill --text "模块即将交接给其他团队维护"'` -> PASS
...
3. 若安装后出现问题，回滚到 `/home/aiot03/.codex/.gdk-backups/20260502T104514Z`。
