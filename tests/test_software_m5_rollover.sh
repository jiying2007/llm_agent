...
for line in (root / "adk.lock").read_text(encoding="utf-8").splitlines():
    if "=" in line:
        key, value = line.split("=", 1)
        lock[key] = value
current_version = lock["agent-dev-kit.version"]
current_commit = lock["agent-dev-kit.commit"]
version_slug = current_version.replace(".", "_").replace("-", "_").replace("+", "_")

assert summary["status"] == "pass", summary
assert summary["candidate_version"] == current_version, summary
...
assert policy["release"]["candidate_version"] == current_version, policy
assert policy["release"]["candidate_commit"] == current_commit, policy
...
assert current_version in Path(summary["qualification_record"]).name, summary
assert scorecard["software_m5"]["candidate_version"] == current_version, scorecard
assert scorecard["working_candidate"]["version"] == current_version, scorecard
assert f"{version_slug}_release_train" not in scorecard["software_m5"]["advisory_followups"], scorecard
