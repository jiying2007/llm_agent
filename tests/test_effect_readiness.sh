...
comparison=compare_effect_trials(fixture.document(), manifest)
...
for index,(kind,asset_id) in enumerate(sorted(assets)):
    ...
    asset_measurements.append({
        ...
        "source_trace_refs":[ref(token+":trace")],
        "asset_bundle_sha256s":[hashlib.sha256((token+":bundle").encode()).hexdigest()],
...
echo '[PASS] effect readiness is blocked by empty governed evidence and becomes ready with complete reviewed coverage'
