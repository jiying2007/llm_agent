...
for name in workspace adk codex hub; do
  repo="$TMP_DIR/$name"
  mkdir -p "$repo"
  rtk git -C "$repo" init -q
  rtk git -C "$repo" config user.email test@example.invalid
  rtk git -C "$repo" config user.name test
  printf 'baseline\n' >"$repo/tracked.txt"
  rtk git -C "$repo" add tracked.txt
  rtk git -C "$repo" commit -q -m baseline
done
...
rtk bash "$ROOT/scripts/cross-repo-release-bundle.sh" \
...
cd /tmp
rtk bash "$ROOT/scripts/cross-repo-release-bundle.sh" --help >/dev/null
