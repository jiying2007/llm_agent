...
`llm_agent` 是长期跟踪 AI Coding 参考仓、吸收高价值工程实践，并把稳定实践压实到 `global-dev-kit`（gdk）的工作区。

当前生产目标不是把参考仓内容直接混装进 `~/.codex`，而是走固定闭环：

1. 参考子仓：同步、扫描、评估优秀实现。
2. `subrepos/adoption-matrix.md`：记录 `adopt/observe/reject` 决策与证据。
3. `global-dev-kit`：把采纳项实现为 Agent/Skill/Workflow/runbook/script/test。
4. `~/.codex`：只安装经过 gdk 门禁压实的资产。
...
- `reports/gdk-production-landing-implementation-2026-05-02.md`：gdk 生产级落地记录。
- `global-dev-kit/README.md`：gdk 使用指南。
- `global-dev-kit/docs/codex-agents-integration.md`：`~/.codex/AGENTS.md` 与 gdk 配合指南。

## 维护原则

- 先压实 gdk，再追踪参考子仓更新。
- 不把第三方参考资产直接复制进 `~/.codex`。
- `global-dev-kit/manifest.yaml` 是 gdk Agent/Skill/Profile 的单一事实源。
...
1. 若只修改 gdk 文档：至少运行 `rtk scripts/check-doc-sync.sh .` 与相关 gdk 测试。
2. 若修改 Agent/Skill/Profile：运行 `rtk global-dev-kit/tests/run_all.sh`。
3. 若影响生产部署或 `~/.codex`：运行 `rtk scripts/check-gdk-harden-readiness.sh . --require-pilot`。
4. 若吸收参考仓更新：先确认 gdk 门禁通过，再执行同步和 diff scan。
