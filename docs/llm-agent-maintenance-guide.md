...
| OSS discovery | `rtk scripts/discover-oss-repos.sh . --dry-run --repo example/manual-discovery --out /tmp/oss-discovery-candidates.jsonl` + `rtk tests/test_oss_discovery.sh` | 只能生成候选 ledger，不联网、不 clone、不注册、不吸收 |
