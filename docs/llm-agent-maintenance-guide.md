...
`llm_agent` 是 gdk 的上游治理工作区，负责三件事：

1. 管理参考子仓清单与同步策略。
2. 从参考子仓提炼可采纳实践，并记录采纳/观察/拒绝原因。
3. 推动 `global-dev-kit` 落地、验证、部署到 `~/.codex`，再把真实运行结果回灌。

不推荐在 `llm_agent` 中直接改 `~/.codex` 运行资产。生产运行资产应由 `global-dev-kit/scripts/devkit.sh install` 生成，并由报告记录版本、profile、optional skill、备份路径和健康检查结果。
...
| `subrepos/phase-gate.env` | 是否允许追踪上游更新 | 默认先压实 gdk，再开门同步 |
| `scripts/` | 治理、同步、门禁脚本 | 新脚本必须写入 `scripts/README.md` |
| `reports/` | pilot、安装、周报、wave 记录 | 生产结论必须有报告证据 |
| `global-dev-kit/` | gdk 源资产与测试 | 所有生产能力最终在这里压实 |
...
适用场景：修改了 gdk 文档、profile、skill、agent、workflow 或 root scripts 后，需要确认基础门禁。
...
适用场景：准备声明 gdk 可用于 `~/.codex` 生产运行，或准备重新部署到 `~/.codex`。

该门禁会检查：

- gdk strict validation
- optional skills 回归
- 外部仓库引用隔离
- skill metadata
...
| 变更类型 | 最小验证 | 说明 |
|---|---|---|
| 文档说明 | `rtk scripts/check-doc-sync.sh .` | 若涉及 gdk docs，还应跑 gdk 相关测试 |
| runbook / workflow 文档 | `rtk global-dev-kit/tests/run_all.sh` | 防止 catalog、格式、引用漂移 |
| Agent/Skill 内容 | `rtk global-dev-kit/tests/run_all.sh` | 会覆盖 frontmatter、内容质量、触发矩阵 |
| profile / manifest | `rtk global-dev-kit/tests/test_profile_coherence.sh` + `rtk global-dev-kit/tests/run_all.sh` | 防止继承重复与未知引用 |
| install / convert 脚本 | `rtk global-dev-kit/tests/run_all.sh` | 必须覆盖安装、转换、dry-run |
| root 门禁脚本 | `rtk scripts/check-gdk-harden-readiness.sh . --require-pilot` | 影响生产放行链路 |
| `~/.codex` 部署 | `rtk scripts/check-gdk-harden-readiness.sh . --require-pilot` + install report | 必须保留 backup |

## 5. 生产部署流程

生产部署只从 `global-dev-kit` 执行，不手工复制单个 Agent/Skill。
...
- 使用 `copy`，避免 gdk 工作区未提交改动影响 `~/.codex`。
- 使用 `--backup`，生成回滚点。
...
```text
/home/aiot03/.codex/.gdk-backups/20260502T104514Z
```
...
## 7. `~/.codex/AGENTS.md` 与 gdk 的关系

`~/.codex/AGENTS.md` 是运行时总策略层，gdk 是 Agent/Skill/Profile 资产供应层。二者不要互相替代。
...
| `~/.codex/agents/` | gdk 安装后的角色 Agent | 手工复制的第三方 Agent |
| `~/.codex/skills/` | gdk 安装后的稳定技能 + 个人已治理技能 | 未审查第三方技能 |
| `llm_agent/global-dev-kit` | 源资产、测试、profile、runbook | 生产运行时的临时状态 |
| `llm_agent/reports` | 安装、pilot、回归、上游吸收证据 | 密钥或私人业务数据 |

`~/.codex/AGENTS.md` 建议保留这些与 gdk 配合的规则：
...
- gdk 资产更新必须先在 `llm_agent/global-dev-kit` 通过回归，再用 `scripts/devkit.sh install` 安装。
...
当前不建议让 gdk 覆盖 `~/.codex/AGENTS.md`。原因：

- `AGENTS.md` 包含个人环境硬约束，如 `rtk` 命令前缀、沟通偏好、文档目录、并行策略。
- gdk 安装器的职责是分发 `agents/` 与 `skills/`，不是替换个人全局策略。
- 若要把 gdk 的策略沉淀进 `~/.codex/AGENTS.md`，应采用“追加小节 + 人工审阅 + 健康检查”的方式。
...
可以同步，但必须先确认 phase gate。默认策略是先压实 gdk，再追踪更新。
...
当前 gdk 是生产分发层和压实层，不是参考仓全文镜像。参考仓继续作为上游灵感和候选池；只有经过采纳矩阵、gdk 实现、门禁和 pilot 的能力才进入生产。
