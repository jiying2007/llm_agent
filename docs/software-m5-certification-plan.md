...
- RC 版本：`agent-dev-kit 3.1.0-rc.4`。
...
| P1 M5-ready RC | implemented | rc.3 升级到 rc.4 后可回退；exact-commit 双构建一致；隔离 wheel 安装通过；自试点已真实启动 | `release-rehearsal.json`、事件链 |
...
rtk bash agent-dev-kit/scripts/devkit.sh eval campaign plan \
  --contract agent-dev-kit/manifests/software_m5_eval_contract_rc4.json \
...
rtk bash agent-dev-kit/scripts/devkit.sh eval campaign run \
  --contract agent-dev-kit/manifests/software_m5_eval_contract_rc4.json \
  --state-dir agent-dev-kit/docs/changes/external-practice-intake-v1/software-m5-campaign-state \
